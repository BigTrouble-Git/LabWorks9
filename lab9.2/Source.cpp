...
int BinSearch(Student* p, const int N, const string prizv, double MathG, double average);
...
{ // забезпечення відображення кирилиці:
	...
	do {
		...
		case 4:
			...
			cin >> average;
			if ((found = BinSearch(p, N, prizv, MathG, average)) != -1)
	...
	for (int i0 = 0; i0 < N - 1; i0++) // метод "бульбашки"
		for (int i1 = 0; i1 < N - i0 - 1; i1++)
		{
			...
			if (avg1 < avg2
				||
				(avg1 == avg2 &&
					p[i1].matematyka < p[i1 + 1].matematyka)
				||
				(avg1 == avg2 &&
					p[i1].matematyka == p[i1 + 1].matematyka &&
...
{ 	
		...
		double avg;
		{
			...
			do {
				...
				if (p[m].prizv == prizv && avg == average && p[m].matematyka == MathG)
					return m;
				if ((avg < average)
					||
					(avg == average &&
						p[m].matematyka < MathG)
					||
					(avg == average) &&
					p[m].matematyka == MathG &&
					p[m].prizv < prizv)
...
double avgIi, avgIj;
	...
	for (i = 1; i < N; i++)
	{
		...
		for (j = i - 1,
			avgIj = ((p[I[j]].fizyka + p[I[j]].matematyka + p[I[j]].programuvanna) / 3.0);
			j >= 0 && ((avgIj > avgIi) ||
				(avgIj == avgIi &&
					p[I[j]].matematyka < p[value].matematyka) ||
				(avgIj == avgIi &&
					p[I[j]].matematyka == p[value].matematyka) &&
